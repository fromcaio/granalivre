...
export default function DeleteAutomationModal({ onClose, automation, onDeleted, title = "Excluir Automação" }) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setError(err.message || "Erro ao excluir a automação.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white w-full max-w-sm rounded-lg shadow-xl p-6 relative">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Tem certeza que deseja excluir <strong>{automation?.name}</strong>?<br />
